...
let handpose;
let predictions = [];
let gameState = 'COVER'; // COVER, QR_WAIT, INTRO, COUNTDOWN, L1, L2, L3, FAIL, SUCCESS
let score = 0;
let levelStep = 0;
let timer = 0;
...
function draw() {
  ...
  if (!video) {
    fill(C_BLUE);
    text("等待手機連線鏡頭...", width/2, height/2);
    return;
  }
...
function drawHuman(x, y, scale, shirtCol) {
  push();
  translate(x, y);
  scale(scale);
  noStroke();
...
function nextL2Question() {
  ...
  currentQuestion = {
    type: 'HOUSE',
    diffIdx: diffIdx, // 0:上, 1:下, 2:左, 3:右
    baseCol: baseCol,
    diffCol: diffCol,
    startTime: millis()
  };
...
function drawLevel2() {
  ...
  let positions = [
    {x: width/2, y: height/4},    // 0: 上
    {x: width/2, y: 3*height/4},  // 1: 下
    {x: width/4, y: height/2},    // 2: 左
    {x: 3*width/4, y: height/2}   // 3: 右
  ];
